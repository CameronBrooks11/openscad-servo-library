...
function ServoLib_BodyHeight(servo_model) =
  ServoLib_AftHeight(servo_model) + ServoLib_WingHeight(servo_model) + ServoLib_ForeHeight(servo_model);
...
function _ServoLib_RetrieveParameter(servo_model, key) =
  ...
  ) ServoLib_ServoModelIsValid(servo_model) == false ? assert(false, str("Servo model \"", servo_model, "\" is not currently supported by servo_lib"))
  : value_index == [] || value == undef ? assert(false, str("Requested parameter \"", key, "\" not found for servo model \"", servo_model, "\""))
  : value;
